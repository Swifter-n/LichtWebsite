...
if(isset($_POST['update'])){
    ...
    if(move_uploaded_file($tmp_name, "../img/".$file_name)){
        mysqli_query($db, "UPDATE website SET title = '$title', description = '$description', link = '$link', image = '$file_name' WHERE id = '$web_id'");
    }else{
        mysqli_query($db, "UPDATE website SET title='$title', description = '$description', link = '$link' WHERE id = '$web_id'");     
    }
    header("location:index.php?website");
}

$edit = $_GET['website-update'];
$edit_query = mysqli_query($db, "SELECT * FROM website WHERE id = '$edit'");
