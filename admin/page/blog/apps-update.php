...
if(isset($_POST['update'])){
    ...
    if(move_uploaded_file($tmp_name, "../img/".$file_name)){
        mysqli_query($db, "UPDATE apps SET title = '$title', description = '$description', link = '$link', image = '$file_name' WHERE id = '$web_id'");
    }else{
        mysqli_query($db, "UPDATE apps SET title='$title', description = '$description', link = '$link' WHERE id = '$web_id'");     
    }
    header("location:index.php?apps");
}

$edit = $_GET['apps-update'];
$edit_query = mysqli_query($db, "SELECT * FROM apps WHERE id = '$edit'");
$row_edit = mysqli_fetch_assoc($edit_query);

$web = mysqli_query($db, "SELECT * FROM apps ORDER BY id DESC");
?>
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Blog &raquo; Product Application</h1>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <?php if(mysqli_num_rows($web)){?>
                            ...
                            <tr>
                                ...
                                <td class="center"><a href="index.php?apps-update=<?=$row_web['id'];?>" class="btn btn-primary btn-xs" type="button">Update</a></td>
                                <td class="center"><a href="index.php?apps-delete=<?=$row_web['id'];?>" class="btn btn-primary btn-xs" type="button">Delete</a></td>
                            </tr>
                            <?php } ?>
                            <?php } ?>
                        </tbody>
